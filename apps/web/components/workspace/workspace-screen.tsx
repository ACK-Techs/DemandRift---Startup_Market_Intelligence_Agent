...
function Head({ screen }: { screen: WorkspaceScreen }) { const copy = screenCopy[screen]; return <header className="motion-enter flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[.13em] text-[var(--brand-deep)]">{copy.eyebrow}</p><h1 className="mt-2 text-[27px] font-semibold tracking-[-.055em] text-[var(--ink)] sm:text-[30px]">{copy.title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#686973]">{copy.description}</p></div><button className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[var(--brand)] px-3.5 py-2.5 text-xs font-semibold text-white transition hover:bg-[var(--brand-deep)] sm:w-auto sm:self-auto" type="button">{copy.action}<ArrowRight className="h-3.5 w-3.5" /></button></header>; }
