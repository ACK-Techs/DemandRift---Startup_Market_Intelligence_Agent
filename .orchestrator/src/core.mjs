...
export async function createRun({ id, title, goal, outputDirectory, repoRoot = REPO_ROOT, actor = 'pm-manager' }) {
  ...
  const run = {
    ...
    constraints: [
      ...
      'Commit onayı varsa her başarıyla tamamlanan write item atomik Conventional Commit ile kaydedilir; mesaj type(scope): summary biçiminde ve aşamaya/modüle uygun olur. Implementer push yapmaz; ayrı push onayıyla kabul edilen integration checkpoint\'ini üst manager push eder.',
      'Faz 8 kullanıcı açıkça başlatmadan tasarlanmaz veya uygulanmaz.',
      'AI source policy, citation, tenant, budget veya decision hard gate kurallarını geçersiz kılamaz.'
    ],
...
export async function renderPrompt(run, itemId, platform, repoRoot = REPO_ROOT) {
  ...
  return `# Orchestrator Handoff\n\nRun: ${run.runId} — ${run.title}\nItem: ${item.id} — ${item.title}\nPlatform: ${adapter.displayName}\n\n` +
    ...
    `## Proje kuralları\n\n${bullets(asArray(run.constraints))}\n- Kapsam dışı refactor yapma. Run commit onayı taşıyorsa write item kontrolleri geçince yalnız atanmış scope'u atomik Conventional Commit ile kaydet; mesajı type(scope): summary biçiminde yaz ve work item ID'sini body/footer'a ekle. Push yapma. Kritik belirsizlikte blocker sonucu üret.\n\n` +
